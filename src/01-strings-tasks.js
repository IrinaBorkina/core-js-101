...
function getRectangleString(width, height) {
  const horizLine = '─'.repeat(width - 2);
  const spaceLine = ' '.repeat(width - 2);
  const topLine = `┌${horizLine}┐\n`;
  const middle = (`│${spaceLine}│\n`).repeat(height - 2);
  const bottomLine = `└${horizLine}┘\n`;
...
function encodeToRot13(str) {
  ...
  return str.replace(/[A-Za-z]/g, (s) => alphabet.charAt(encodedAlphabet.indexOf(s)));
}
