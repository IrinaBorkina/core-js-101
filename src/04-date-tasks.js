...
function isLeapYear(date) {
  ...
  if ((dat % 4 !== 0 || (dat % 100 === 0)) && dat % 400 !== 0) return false;
  return true;
}
...
function angleBetweenClockHands(date) {
  ...
  const hours = 0.5 * (60 * hour + date.getUTCMinutes()); // the angle of the hour hand
  const minutes = 6 * date.getUTCMinutes(); // the angle of the minute hand
  //  the angle between the hands
  const difference = hours - minutes > 180 ? hours - minutes - 180 : hours - minutes;
  return (Math.PI * Math.abs(difference)) / 180; // degrees to radians
}
